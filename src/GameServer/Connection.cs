...
internal partial class Connection
{
    ...
    public static readonly List<OpCode> BANNED_PACKETS = new() { OpCode.WindSeedClientNotify, OpCode.PlayerLuaShellNotify };
#if DEBUG
    ...
    public static void LogPacket(string sendOrRecv, ushort opcode, byte[] payload)
    {
       ...
       SingleOrDefault(assembly => assembly.GetName().Name == "Shared").GetTypes().First(t => t.Name == $"{Enum.GetName(typeof(OpCode), opcode)}"); //get the type using the packet name
        ...
        JsonFormatter? formatter = Google.Protobuf.JsonFormatter.Default;
        string? asJson = formatter.Format(packet);
        Logger.DebugWriteLine($"{sendOrRecv}: {Enum.GetName(typeof(OpCode), opcode)}({opcode})\r\n{asJson}");
        if (GameServer.Configuration.Server.LogOptions.KeepLog)
        {
            File.WriteAllText($"{GameServer.Configuration.Server.LogOptions.LogLocation}\\{LogIndex++}_Packet_{packet.GetType().Name}.json", JValue.Parse(asJson).ToString(Formatting.Indented));
        }
    }

    internal static void LogCombatInvocation(string sendOrRecv, CombatInvokeEntry entry, Type invocType)
    {
        MessageDescriptor? descriptor = (MessageDescriptor)invocType.GetProperty("Descriptor", BindingFlags.Public | BindingFlags.Static).GetValue(null, null); // get the static property Descriptor
        IMessage? invoc = descriptor.Parser.ParseFrom(entry.CombatData);
        JsonFormatter? formatter = Google.Protobuf.JsonFormatter.Default;
        string? asJson = formatter.Format(invoc);
        Logger.DebugWriteLine($"{sendOrRecv} {entry.ForwardType} | {entry.ArgumentType}: \r\n{asJson}");
        if (GameServer.Configuration.Server.LogOptions.KeepLog)
        {
            File.WriteAllText($"{GameServer.Configuration.Server.LogOptions.LogLocation}\\{LogIndex++}_CInv_{entry.ArgumentType}.json", JValue.Parse(asJson).ToString(Formatting.Indented));
        }
    }
    ...
    {
        MessageDescriptor? descriptor = (MessageDescriptor)invocType.GetProperty("Descriptor", BindingFlags.Public | BindingFlags.Static).GetValue(null, null); // get the static property Descriptor
        IMessage? invoc = descriptor.Parser.ParseFrom(entry.AbilityData);
        JsonFormatter? formatter = Google.Protobuf.JsonFormatter.Default;
        string? asJson = formatter.Format(invoc);
        string? headJson = formatter.Format(entry.Head);
        Logger.DebugWriteLine($"{sendOrRecv} {entry.Head} | {entry.ForwardType} | {entry.ArgumentType}: \r\n{asJson}");
        if (GameServer.Configuration.Server.LogOptions.KeepLog)
        {
            File.WriteAllText($"{GameServer.Configuration.Server.LogOptions.LogLocation}\\{LogIndex++}_AInv_Head_{entityId}_{entry.ArgumentType}.json", JValue.Parse(headJson).ToString(Formatting.Indented));
            File.WriteAllText($"{GameServer.Configuration.Server.LogOptions.LogLocation}\\{LogIndex++}_AInv_{entityId}_{entry.ArgumentType}.json", JValue.Parse(asJson).ToString(Formatting.Indented));
        }
    }
#endif
    ...
    private async Task ProcessMessageAsync(Memory<byte> data)
    {
        ...
        {
#if DEBUG
            bool allDebug = GameServer.Configuration.Server.LogOptions.LogPackets == PacketDebugMode.ALL;
#endif
            while (br.BaseStream.Position < br.BaseStream.Length)
            {
                ...
                else if (debugMode == PacketDebugMode.WHITELIST && GameServer.Configuration.Server.LogOptions.DebugWhitelist.Contains((OpCode)opcode))
                {
                    goto Log;
                }
                else if (debugMode == PacketDebugMode.BLACKLIST && !GameServer.Configuration.Server.LogOptions.DebugBlacklist.Contains((OpCode)opcode))
                {
                    goto Log;
                }
                else
    ...
    private async Task<bool> HandlePacketAsync(ushort opcode, byte[] header, byte[] payload)
    {
        ...
        if (GameServer.Configuration.Server.LogOptions.LogPackets == PacketDebugMode.MISSING)
...
#if DEBUG
        // Log
        switch (GameServer.Configuration.Server.LogOptions.LogPackets)
        {
            case PacketDebugMode.ALL:
                {
                    LogPacket("SEND", packet.Opcode, packet.Data);
                    break;
                }
            case PacketDebugMode.WHITELIST:
                {
                    if (GameServer.Configuration.Server.LogOptions.DebugWhitelist.Contains((OpCode)packet.Opcode))
                    {
                        LogPacket("SEND", packet.Opcode, packet.Data);
                    }
                    break;
                }
            case PacketDebugMode.BLACKLIST:
                {
                    if (!GameServer.Configuration.Server.LogOptions.DebugBlacklist.Contains((OpCode)packet.Opcode))
                    {
                        LogPacket("SEND", packet.Opcode, packet.Data);
                    }
                    break;
    ...
    private async Task SetSecretKey(ulong seed)
    {
        Crypto.MT19937? mt = new(seed);
        mt.Seed(mt.Int63());
        mt.Int63();

        await using MemoryStream? ms = new(0x1000);
