...
namespace Weedwacker.GameServer.Systems.Inventory
{
    internal class MaterialSubInv : SubInventory
    {
        ...
        {
            FoodTab = new FoodTab(owner, inventory);
            PromoteTab = new PromoteTab(owner, inventory);
            MaterialsTab = new MaterialsTab(owner, inventory);
            GadgetTab = new GadgetTab(owner, inventory);
            QuestTab = new QuestTab(owner, inventory);
            PreciousTab = new PreciousTab(owner, inventory);
            BattlePassTab = new BattlePassTab(owner, inventory);
        }
            ...
            foreach (ItemUseConfig op in data.itemUse)
            {
                if (op.useOp != Data.Enums.ItemUseOp.ITEM_USE_NONE)
                {
                    IEnumerable<Type>? query =
                        ...
                        select type;
                    Type? opType = query.FirstOrDefault();
