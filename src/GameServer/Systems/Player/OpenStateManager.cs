...
internal class OpenStateManager
{
	...
	public static HashSet<uint> DEFAULT_OPEN_STATES = new(GameData.OpenStateDataMap.Where(w => w.Value.defaultState == true).Select(s => s.Key));
	...
	public async Task SetOpenStateAsync(uint openState, int value, bool sendNotify = true)
	{
		...
		if (value != previousValue)
		{
			...
			// Update Database
			FilterDefinition<ProgressManager>? filter = Builders<ProgressManager>.Filter.Where(w => w.OwnerId == Owner.GameUid);
			UpdateDefinition<ProgressManager>? update = Builders<ProgressManager>.Update.Set($"{nameof(ProgressManager.OpenStates)}.{(OpenStateType)openState}", value);
	...
	**********/
	private bool AreConditionsMet(OpenStateData openState)
	{
		...
		foreach (OpenStateData.OpenStateCond? condition in openState.cond)
		{
			switch (condition.cond_type)
			{
				...
				case OpenStateCondType.OPEN_STATE_COND_PLAYER_LEVEL:
					{
						if (Owner.PlayerProperties[PropType.PROP_PLAYER_LEVEL] < condition.param)
						{
							return false;
						}
						else continue;
	...
	**********/
	public async Task TryUnlockOpenStatesAsync(bool sendNotify)
	{
		...
		IEnumerable<KeyValuePair<uint, OpenStateData>>? lockedStates = GameData.OpenStateDataMap.Where(s => Owner.ProgressManager.OpenStates.GetValueOrDefault((OpenStateType)s.Key, 0) == 0);

		// Try unlocking all of them.
		foreach (KeyValuePair<uint, OpenStateData> state in lockedStates)
		{
