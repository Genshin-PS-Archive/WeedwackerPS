...
namespace Weedwacker.GameServer.Systems.Player
{
    internal class TeamManager : BaseEntity
    {
        ...
        [BsonIgnore] public TeamInfo CurrentTeamInfo => Owner.IsInMultiplayer ? MpTeam : Teams[CurrentTeamIndex];

        [BsonIgnore] public TeamInfo CurrentSinglePlayerTeamInfo => Teams[CurrentTeamIndex];
        [BsonIgnore] public TeamInfo MpTeam;
        [BsonIgnore] public AvatarEntity? CurrentAvatarEntity => ActiveTeam.GetValueOrDefault(CurrentCharacterIndex); //prevent exception on logout
        ...
        public uint MaxTeamSize
        {
            get
            {
                ...
                {
                    uint max = GameServer.Configuration.Server.GameOptions.AvatarLimits.MultiplayerTeam;
                    if (Owner.World.Host == Owner)
                    {
                        return Math.Max(1, (uint)Math.Ceiling(max / (double)Owner.World.Players.Count));
                    }
                    return Math.Max(1, (uint)Math.Floor(max / (double)Owner.World.Players.Count));
                }
                else
        ...
        public async Task OnLoadAsync(Player owner)
        {
            Owner = owner;
            AbilityManager = new TeamAbilityManager(this);
            ActiveTeam = new SortedList<int, AvatarEntity>();
            // Point to the "real" avatars
            SortedList<int, TeamInfo>? reloadedTeams = new();
            foreach (KeyValuePair<int, TeamInfo> team in Teams)
            {
                reloadedTeams.Add(team.Key, new TeamInfo(team.Value.TeamName));
                foreach (KeyValuePair<int, Avatar.Avatar> entry in team.Value.AvatarInfo)
                {
            ...
            foreach (KeyValuePair<int, Avatar.Avatar> characterIndex in Teams[CurrentTeamIndex].AvatarInfo)
            {
        ...
        public async Task UpdateTeamEntitiesAsync()
        {
            ...
            foreach (KeyValuePair<int, AvatarEntity> entity in ActiveTeam)
            {
            ...
            // Update avatar entities of new active team in scene
            await Owner.Scene.UpdatePlayerAvatarsAsync(Owner, previousTeam.Values);
        ...
        public async Task SetupMpTeamAsync(List<ulong> list)
        {
            ...
            MpTeam = new TeamInfo(newTeam, MpTeam.TeamName);
        }
