...
internal class Player
{
    ...
    [BsonElement("Position")] public float[] PositionArray { get => new[] { Position.X, Position.Y, Position.Z }; set => Position = new Vector3(value[0], value[1], value[2]); }
    [BsonElement("Rotation")] public float[] RotationArray { get => new[] { Rotation.X, Rotation.Y, Rotation.Z }; set => Rotation = new Vector3(value[0], value[1], value[2]); }
    public uint NextResinRefresh;
    public int LastDailyReset;
    public Dictionary<PropType, int> PlayerProperties { get; set; } = new(); // SET ONLY THROUGH THE PROPMANAGER
    ...
    [BsonIgnore] public PlayerLocationInfo PlayerLocationInfo => new() { Uid = (uint)GameUid, Pos = new Vector { X = Position.X, Y = Position.Y, Z = Position.Z }, Rot = new Vector { X = Rotation.X, Y = Rotation.Y, Z = Rotation.Z } };

    public Player(string heroName, string accountUid, uint gameUid)
    {
        Profile = new PlayerProfile(heroName);

        AccountUid = accountUid;
        BattlePassManager = new BattlePassManager(this);
        EnergyManager = new EnergyManager(this);
        ExpManager = new ExpManager(this);
        GadgetManager = new ClientGadgetEntityManager(this);
        GameUid = gameUid;
        ProgressManager = new ProgressManager(this);
        MapMarksManager = new MapMarksManager(this);
        PropManager = new PlayerPropertyManager(this);
        ResinManager = new ResinManager(this);
        ShopManager = new ShopManager(this);
        SocialManager = new SocialManager(this);
        OpenStateManager = new OpenStateManager(this);
        //StaminaManager = new StaminaManager(this);
        CombatInvNotifyList = new InvokeNotifier<CombatInvokeEntry>(this, typeof(PacketCombatInvocationsNotify));
        AbilityInvNotifyList = new InvokeNotifier<AbilityInvokeEntry>(this, typeof(PacketAbilityInvocationsNotify));
        ClientAbilityInitFinishNotifyList = new InvokeNotifier<AbilityInvokeEntry>(this, typeof(PacketClientAbilityInitFinishNotify));
    }

    private async Task OnCreate()
    {
        await PropManager.SetPropertyAsync(PropType.PROP_PLAYER_LEVEL, 1, false);
        await PropManager.SetPropertyAsync(PropType.PROP_PLAYER_WORLD_LEVEL, 0, false);
        await PropManager.SetPropertyAsync(PropType.PROP_IS_SPRING_AUTO_USE, 1, false);
        await PropManager.SetPropertyAsync(PropType.PROP_SPRING_AUTO_USE_PERCENT, 50, false);
        await ResinManager.AddResinAsync(160);
        await PropManager.SetPropertyAsync(PropType.PROP_IS_FLYABLE, 0, false);
        await PropManager.SetPropertyAsync(PropType.PROP_MAX_STAMINA, 10000, false);
        await PropManager.SetPropertyAsync(PropType.PROP_CUR_PERSIST_STAMINA, 10000, false);
        await PropManager.SetPropertyAsync(PropType.PROP_IS_TRANSFERABLE, 1, false);
    ...
    public static uint GetGameUidByGameGuid(ulong guid)
    {
        return (uint)(guid >> 32);
    }
    ...
    public async Task<bool> SetMainCharacter(uint avatarId, string heroName)
    {
        if (GameData.AvatarHeroEntityDataMap.ContainsKey(avatarId) && MainCharacterId == 0)
        {
            ...
            Profile.HeadImage = new ProfilePicture { AvatarId = (uint)avatarId, CostumeId = 0 };

            // Update Database
            FilterDefinition<Player>? filter = Builders<Player>.Filter.Where(w => w.AccountUid == AccountUid);
            UpdateDefinition<Player>? update = Builders<Player>.Update.Set(w => w.Profile, Profile).Set(w => w.MainCharacterId, avatarId);
    ...
    public async Task SetSceneAsync(Scene? scene)
    {
        ...
        // Update Database
        FilterDefinition<Player>? filter = Builders<Player>.Filter.Where(w => w.AccountUid == AccountUid);
        UpdateDefinition<Player>? update = Builders<Player>.Update.Set(w => w.SceneId, SceneId);
        await DatabaseManager.UpdatePlayerAsync(filter, update);
    }
    ...
    public async Task EnterWorldAreaAsync(uint areaType, uint areaID, bool isInit = false)
    {
        ...
        // Update Database
        FilterDefinition<Player>? filter = Builders<Player>.Filter.Where(w => w.AccountUid == AccountUid);
        UpdateDefinition<Player>? update = Builders<Player>.Update.Set(w => w.WorldAreaIds, WorldAreaIds);
        await DatabaseManager.UpdatePlayerAsync(filter, update);
    }
    ...
    public async void OnTickAsync()
    {
        ...
        if (World != null)
        {
            ...
            if (Scene != null && time > NextSendPlayerTimeNotify)
            {
                ...
                FilterDefinition<Player>? filter = Builders<Player>.Filter.Where(w => w.AccountUid == AccountUid);
                UpdateDefinition<Player>? update = Builders<Player>.Update.Set(w => w.PositionArray, PositionArray).Set(w => w.RotationArray, RotationArray);
                await DatabaseManager.UpdatePlayerAsync(filter, update);
            }
        }
        // Recharge resin.
    ...
    public async Task OnLoginAsync()
    {
        ...
        // Multiplayer setting
        await PropManager.SetPropertyAsync(PropType.PROP_PLAYER_MP_SETTING_TYPE, (int)MpSettingType.EnterAfterApply, false);
        await PropManager.SetPropertyAsync(PropType.PROP_IS_MP_MODE_AVAILABLE, 1, false);
    ...
    public OnlinePlayerInfo GetOnlinePlayerInfo()
    {
        OnlinePlayerInfo onlineInfo = new()
        {
            ...
            PlayerLevel = (uint)PlayerProperties[PropType.PROP_PLAYER_LEVEL],
            MpSettingType = (MpSettingType)PlayerProperties[PropType.PROP_PLAYER_MP_SETTING_TYPE],
