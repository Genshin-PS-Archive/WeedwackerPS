...
internal class ResinManager
{
	...
	public async Task<bool> UseResinAsync(int amount)
	{
		...
		int currentResin = Owner.PlayerProperties[PropType.PROP_PLAYER_RESIN];
		...
		await Owner.PropManager.SetPropertyAsync(PropType.PROP_PLAYER_RESIN, newResin);
		...
		{
			uint currentTime = (uint)(DateTimeOffset.Now.ToUnixTimeMilliseconds() / 1000);
	...
	public async Task<bool> AddResinAsync(int amount)
	{
		...
		// Add resin.
		int currentResin = Owner.PlayerProperties.GetValueOrDefault(PropType.PROP_PLAYER_RESIN);
		int newResin = currentResin + amount;

		if (await Owner.PropManager.SetPropertyAsync(PropType.PROP_PLAYER_RESIN, newResin))
		{
	...
	public async Task RechargeResin()
	{
		...
		uint currentResin = (uint)Owner.PlayerProperties.GetValueOrDefault(PropType.PROP_PLAYER_RESIN);
		uint currentTime = (uint)(DateTimeOffset.Now.ToUnixTimeMilliseconds() / 1000);
		...
		uint recharge = 1 + (uint)((currentTime - Owner.NextResinRefresh) / GameServer.Configuration.Server.GameOptions.ResinOptions.RechargeTime);
		uint newResin = Math.Min(GameServer.Configuration.Server.GameOptions.ResinOptions.Cap, currentResin + recharge);
		uint resinChange = newResin - currentResin;

		await Owner.PropManager.SetPropertyAsync(PropType.PROP_PLAYER_RESIN, (int)newResin);
		...
		{
			uint nextRecharge = Owner.NextResinRefresh + resinChange * GameServer.Configuration.Server.GameOptions.ResinOptions.RechargeTime;
	...
	public async Task OnLoginAsync()
	{
		...
		if (!GameServer.Configuration.Server.GameOptions.ResinOptions.ResinUsage)
		{
			await Owner.PropManager.SetPropertyAsync(PropType.PROP_PLAYER_RESIN, (int)GameServer.Configuration.Server.GameOptions.ResinOptions.Cap);
			Owner.NextResinRefresh = 0;
		}
		else
		{
