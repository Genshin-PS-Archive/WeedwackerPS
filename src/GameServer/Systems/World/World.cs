...
namespace Weedwacker.GameServer.Systems.World
{
    internal class World : BaseEntity // Yes... the world is an entity...
    {
        ...
        public Scene? GetSceneById(uint sceneId)
        {
            // Get scene normally
            ...
            if (sceneData != null)
            {
                scene = Scene.CreateAsync(this, sceneData);
        ...
        {
            if (!GameData.SceneDataMap.TryGetValue(sceneId, out _))
                return false;
            ...
            {
                SceneInfo? sceneInfo = GameData.GetSceneScriptsAsync(sceneId);
                if (sceneInfo == null)
                    return false;

                teleportTo = sceneInfo.scene_config?.born_pos ?? Vector3.Zero;
                player.Rotation = sceneInfo.scene_config?.born_rot ?? Vector3.Zero;
            }
        ...
        public async Task<bool> OnTickAsync()
        {
            ...
            Scenes.AsParallel().WithDegreeOfParallelism(1).ForAll(async x => await x.Value.OnTickAsync());
            return true;
        }
    }
}
