...
namespace Weedwacker.GameServer.Systems.Shop
{
    internal class Shop
    {
        ...
        public Shop(Player.Player customer, int shopType)
        {
            Customer = customer;
            ShopType = shopType;
            List<ShopGoodsData>? allGoods = GameData.ShopGoodsDataMap.Values.Where(w => w.shopType == shopType).ToList();
        ...
        public async Task CheckRefreshTime()
        {
            ...
            if (NextRefreshTime <= nextRefresh)
            {
                ...
                //Update database
                FilterDefinition<ShopManager>? filter1 = Builders<ShopManager>.Filter.Where(w => w.OwnerId == Customer.GameUid);
                UpdateDefinition<ShopManager>? update1 = Builders<ShopManager>.Update.Set(w => w.Shops[ShopType], this);
