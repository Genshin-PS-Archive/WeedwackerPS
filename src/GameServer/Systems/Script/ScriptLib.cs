...
public static class ScriptLib
{
	...
	public static int ChangeGroupGadget(ScriptLibContext context, object table_)
	{
		...
		int configId = (int)(long)table["config_id"];
		int state = (int)(long)table["state"];

		IEnumerable<IScriptEntity>? entity = context.SceneScriptManager.Scene.ScriptEntities.Values.Where(w => w.ConfigId == configId);
		if (entity == null)
		{
			return 1;
		}
	...
	public static int ChangeGroupVariableValue(ScriptLibContext context, string var, int value)
	{
		...
		int oldVal = context.SceneScriptManager.GetGroupVariable(context.CurrentGroup.BlockId, context.CurrentGroup.GroupId, var);
		context.SceneScriptManager.SetGroupVariable(context.CurrentGroup.GroupId, var, oldVal + value);
		return 0;
	}
	public static int CheckRemainGadgetCountByGroupId(ScriptLibContext context, object table_)
	{
		...
		int count = context.SceneScriptManager.Scene.ScriptEntities.Values.Where(w => w is ScriptGadgetEntity gadget && gadget.GroupId == groupId).Count();
		return count;
	}
	public static int CreateGadget(ScriptLibContext context, object table_)
	{
		...
		SceneGroup? group = context.CurrentGroup;
		...
		KeyValuePair<uint, SceneGroup.Gadget> gadget = group.gadgets.Where(w => w.Value.config_id == configId).First();
		/*
		var entity = context.SceneScriptManager.CreateGadget(group.GroupId, group.BlockId, gadget);
	...
	public static int CreateMonster(ScriptLibContext context, object table_)
	{
		LuaTable? table = table_ as LuaTable;
		int configId = (int)(long)table["config_id"];
		int delayTime = (int)(long)table["delay_time"];
	...
	public static int ExecuteGroupLua(ScriptLibContext context, int group_id, string local_function_name, object table_)
	{
		//TODO
		return 0;
	}
	...
	public static int GetGadgetStateByConfigId(ScriptLibContext context, int groupId, int configId)
	{
		...
		if (groupId == 0)
		{
			groupId = (int)context.CurrentGroup.GroupId;
		}
		int realGroupId = groupId;
		IScriptEntity? gadget = context.SceneScriptManager.Scene.ScriptEntities.Values
	...
	public static int GetGroupMonsterCount(ScriptLibContext context)
	{
		...
		return context.SceneScriptManager.Scene.ScriptEntities.Values
			.Where(w => w is MonsterEntity && w.GroupId == context.CurrentGroup.GroupId).Count();
	}
	...
	public static int GetGroupVariableValue(ScriptLibContext context, string var)
	{
		...
		return context.SceneScriptManager.GetGroupVariable(context.CurrentGroup.BlockId, context.CurrentGroup.GroupId, var);
	}
	...
	public static int GetHostQuestState(ScriptLibContext context, int questId)
	{
		//TODO
		return 0;
	}
	...
	public static int KillEntityByConfigId(ScriptLibContext context, object table_)
	{
		...
		{
			foreach (IScriptEntity? entity in entities)
			{
	...
	*/
	...
	public static int SetEntityServerGlobalValueByConfigId(ScriptLibContext context, int configId, string sgv, int value)
	{
		//TODO
		return 0;
	}
	...
	public static int SetGroupVariableValue(ScriptLibContext context, string var, int value)
	{
		...
		context.SceneScriptManager.SetGroupVariable(context.CurrentGroup.GroupId, var, value);
		return 0;
	}
	...
	{
		LuaTable? table = options as LuaTable;
