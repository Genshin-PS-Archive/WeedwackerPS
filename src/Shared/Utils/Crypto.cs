...
namespace Weedwacker.Shared.Utils
{
    public class Crypto
    {
        ...
        {
            DispatchEncryptionKeys = new Dictionary<int, RSA>();
            if (!Directory.Exists(path))
                Directory.CreateDirectory(path);
            string? keypath = Path.Combine(path, "dispatchKey.bin");
            string? seedpath = Path.Combine(path, "dispatchSeed.bin");
            string? encryptkeypath = Path.Combine(path, "secretKey.bin");
            string? encryptseedbufferpath = Path.Combine(path, "secretKeyBuffer.bin");
            string? authkeypath = Path.Combine(path, "auth_private-key.pem");
            ...
            try
            {
                ...
                RSAParameters OSparams = Cur3Encryptor.ExportParameters(true);
                ...
                RSAParameters signParams = CurSigner.ExportParameters(true);
        ...
        public static string GetPasswordHash(string encryptedpwd)
        {
            ...
            UTF8Encoding ByteConverter = new();

            RSA.ImportFromPem(ByteConverter.GetString(Crypto.AUTH_KEY));
            try
            {
                byte[]? decryptedpwd = RSA.Decrypt(Convert.FromBase64String(encryptedpwd), false);
                byte[]? ret = new MD5CryptoServiceProvider().ComputeHash(
                    decryptedpwd
                    );
